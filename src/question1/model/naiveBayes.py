            ...
            for j in range(TOTAL_SEQ):
                for k in range(TOTAL_AA):
                    if train_array[i][k + (j * TOTAL_AA)] == "1":
                        if prob_0[j][k] == 0:
                            prob_0[j][k] = (
                                prob_0[j][k] + 1) * (1 / NUM_LAB_0)
                        else:
                            prob_0[j][k] = prob_0[j][k] + (1 / NUM_LAB_0)

    # Calculating the probability of every amino acid in each sequence with label 1.
    # Probability table of entries with label 1
    for i in range(TOTAL_8MER):
        if train_array[i][LABEL] == "1":
            for j in range(TOTAL_SEQ):
                for k in range(TOTAL_AA):
                    if train_array[i][k + (j * TOTAL_AA)] == "1":
                        if prob_1[j][k] == 0:
                            prob_1[j][k] = (
                                prob_1[j][k] + 1) * (1 / NUM_LAB_1)
                        else:
                            prob_1[j][k] = prob_1[j][k] + (1 / NUM_LAB_1)
